...
app.locals.TF_BROWSER = config.TF_BROWSER;
...
if (!app.get('debug')) {
  app.use('/calendar/static', express.static(path.join(__dirname, 'assets/dist')));
  app.use(favicon(path.resolve(__dirname,'assets/dist/favicon.ico')));
}else{
...
// 启动express服务
let PORT = parseInt(process.env.PORT || 9999);
try {
  app.listen(PORT, () => {
